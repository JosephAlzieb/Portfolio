...
import { FiSend, FiCheckCircle, FiAlertCircle, FiTerminal } from "react-icons/fi";
...
const baseInputClasses =
  "w-full px-4 py-3 rounded-xl bg-background/80 border outline-none transition-all duration-300 text-foreground placeholder:text-muted/30 font-mono text-sm";

const inputOk =
  "border-card-border/60 focus:border-primary focus:ring-2 focus:ring-primary/20 focus:shadow-[0_0_24px_-6px] focus:shadow-primary/30";

const inputErr =
  "border-red-500/60 focus:border-red-500 focus:ring-2 focus:ring-red-500/20";

/**
 * Contact section — Terminal/command-line inspired aesthetic.
 * Monospace font accents, prompt-style labels, blinking cursor details.
 */
  ...
  return (
    <section id="contact" className="py-28 px-4 bg-background relative overflow-hidden">
      ...
      <div
        ...
        style={{
          backgroundImage: `repeating-linear-gradient(0deg, var(--foreground) 0, var(--foreground) 1px, transparent 1px, transparent 3px)`,
        }}
      />
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <AnimatedSection delay={0.2}>
          <div className="relative rounded-2xl bg-card/50 backdrop-blur-sm border border-card-border/50 shadow-2xl shadow-primary/5 overflow-hidden">
            {/* Terminal title bar */}
            <div className="flex items-center gap-2 px-5 py-3 bg-card/80 border-b border-card-border/40">
              ...
              <div className="flex-1 text-center">
                <span className="text-[11px] font-mono text-muted/50 tracking-wider">
                  contact@portfolio ~ msg
                </span>
              </div>
              <FiTerminal className="text-muted/30" size={14} />
            </div>
              ...
              <AnimatePresence mode="wait">
                {status === "success" ? (
                  ...
                  >
                    ...
                    <p className="text-muted mb-2 font-mono text-sm">
                  ...
                  >
                    ...
                    {status === "error" && (
                      ...
                      >
                        ...
                        <span className="text-red-400/70">ERR:</span> {t("error_message")}
                      </motion.div>
                    )}
                    ...
                    <button
                      ...
                      className="w-full px-6 py-3.5 bg-primary hover:bg-primary-dark text-white rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/40 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
                    >
                      {status === "sending" ? (
    ...
    <AnimatePresence>
      {message && (
        <motion.p
          ...
          className="text-red-500 text-xs mt-1.5 flex items-center gap-1 font-mono"
        >
